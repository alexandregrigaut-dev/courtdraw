#!/usr/bin/env python3
"""
generate-seo-pages.py
Generates all per-sport SEO landing pages for CourtDraw.
Run: python3 scripts/generate-seo-pages.py
"""
...
FAMILIES = {
  'racquet': {'label': 'Racquet & Net Sports', 'slugs': ['tennis','padel','pickleball','badminton','squash','table-tennis','racquetball','beach-tennis','padbol']},
  'football': {'label': 'Football Codes', 'slugs': ['football','futsal','beach-soccer','rugby-union','rugby-league','gaelic-football','american-football','australian-rules']},
  'hockey': {'label': 'Stick & Hockey Sports', 'slugs': ['field-hockey','ice-hockey','roller-hockey','indoor-hockey','floorball','hurling','lacrosse']},
  'hand': {'label': 'Hand & Goal Team Sports', 'slugs': ['handball','beach-handball','basketball','netball','korfball','water-polo']},
  'volley': {'label': 'Volley & Net Team Sports', 'slugs': ['volleyball','beach-volleyball','fistball']},
}
...
<p>Plays from the tactics library can be loaded directly into the board, modified, and saved under your own club library. Every saved tactic is available offline, so you can coach courtside without a wifi connection.</p>''',
  },
  {
    ...
    'plays': [
      ...
      {'id':'padel_bandeja','name':'Bandeja Smash','desc':'High overhead with spin and control to keep net dominance without over-committing.'},
...
<p>The two foundational tools for creating and closing openings in squash are <strong>length</strong> (deep, tight drives to the back corners) and <strong>width</strong> (cross-court drives that pull the opponent across the court):</p>
<ul>
<li><strong>Tight drive</strong> — a straight drive that "dies" in the back corner, clinging to the side wall and leaving no angle to attack. The ideal contact point and wall distance are much easier to show than describe.</li>
...
<li><strong>Width-then-boast</strong> — a cross-court drive to pull the opponent wide, then a boast into the front corner when they overcommit.</li>
</ul>
<h2>Attacking Shots: Boast, Drop, and Nick</h2>
<p>Three attacking shots create winning openings in squash:</p>
<ul>
<li><strong>Boast</strong> — played off the side wall to the opposite front corner; dangerous if the opponent is caught deep.</li>
...
<li><strong>Nick</strong> — the junction of the side wall and the floor, usually in the front corners. A ball that rolls out of the nick is unplayable. Diagram the target angle for each type of nick.</li>
</ul>
...
<ul>
<li><strong>Pendulum serve</strong> — sidespin serve using a pendulum swing motion producing heavy left-sidespin; returners must adjust blade angle and aim toward the server's backhand to cancel the spin.</li>
<li><strong>Reverse pendulum</strong> — identical windup but opposite sidespin; returners expecting the pendulum are caught completely off guard.</li>
<li><strong>Short push to the crossover point</strong> — targeting the opponent's elbow with a short, heavy backspin serve forces a cramped response.</li>
</ul>
...
<p>The <strong>third ball attack</strong> is table tennis's equivalent of padel's net domination principle — it's the tactical framework that justifies serve selection. After a serve designed to elicit a specific return, the server loops or drives the third ball at the predetermined target. Draw the serve, the anticipated return trajectory, and the third ball placement in one connected diagram.</p>
...
<p>The <strong>center court position</strong> in racquetball — roughly five feet behind the short line in the middle of the court — is the tactical equivalent of squash's T-position. From here, a player can reach any corner of the court in the fewest possible steps. Every shot should either be hit from center court or force the opponent out of center court so you can occupy it. Draw this zone on CourtDraw and it immediately clarifies why players should return there after every exchange.</p>
<h2>Kill Shots and Pinch Shots</h2>
<p>The <strong>kill shot</strong> — aimed at the front wall as low as possible so it barely rebounds and dies before the opponent can retrieve it — is racquetball's most decisive weapon:</p>
<ul>
...
<p>For each, diagram the contact point, the wall target (including the specific height), and the expected rebound path.</p>
<h2>Ceiling Ball Strategy</h2>
<p>The <strong>ceiling ball</strong> is racquetball's defensive reset — hit high onto the ceiling so it bounces off the front wall and dies deep in the back court, forcing the opponent away from center court. Diagram the ceiling contact point (just past the front wall), the arc, and the intended landing zone in the back corners. Show how a ceiling ball aimed at the backhand corner creates a left-to-right footwork obligation for the opponent.</p>
<h2>Z-Ball and Around-the-Wall Ball</h2>
<p>Two advanced shots create unexpected rebound patterns:</p>
<ul>
<li><strong>Z-ball</strong> — hit high into the front wall near one corner, rebounds to the side wall, crosses the court, and hits the opposite side wall before dying near the back wall.</li>
<li><strong>Around-the-wall ball (AWB)</strong> — similar but hit higher; rebounds from the front wall to the side wall, across the ceiling, and into the back corner.</li>
</ul>
...
<p>The overhead smash is the most decisive shot in beach tennis. Diagram the three primary smash targets:</p>
<ul>
...
<p>For each, also draw the expected defensive response and how the smashing pair should follow up.</p>
...
<ul>
<li><strong>Central deep lob</strong> — safest option; lands between the opponents and forces them both to move back.</li>
...
<p>Diagram both lob types and the simultaneous forward movement the lobbing pair should make while the opponents retreat.</p>
<h2>Serve Patterns</h2>
<p>In beach tennis, serve selection focuses on placement over spin because the no-bounce rule means every rally starts at the net. Diagram the three serve zones — wide, body, and T — and show how the serving pair should position immediately after serving to cover the likely return angles. Players who anticipate the return direction will reach the net first and take the initiative for the rally.</p>''',
  },
  {
    'slug': 'padbol',
    'name': 'Padbol',
    ...
    'metaDesc': 'Free online padbol tactics board for coaches. Draw 2v2 positioning, wall-play combinations, and net attack patterns for this padel-football hybrid.',
    ...
    'family': 'racquet',
    'plays': [],
    'intro': "<p>Padbol merges the glass-wall geometry of padel with foot and body striking, creating a unique 2v2 game where positioning and wall-reading are as important as technique. CourtDraw's padbol board helps coaches diagram net dominance principles, explain the wall rebound angles unique to padbol's smaller court, and plan training drills before the session starts.</p>",
...
<p>Padbol's back and side glass walls create rebound combinations that are almost impossible to explain without a diagram. A ball struck at a side glass at a 45-degree angle will rebound across the court and, if hit with the right pace, reach the opposite back glass. Key combinations to diagram:</p>
<ul>
...
<ul>
<li><strong>6-0 defense</strong> — all six defenders on or just inside the 6-metre line, forming a compact wall. Show the spacing (roughly 2–3 metres between each player) and the sliding coordination when the ball moves between positions.</li>
<li><strong>5-1 defense</strong> — one defender moves out to pressure the opponent's centre-back at 9 metres; the remaining five hold the 6-metre line. Show how this changes the spacing requirements for the five back players.</li>
...
<ul>
<li><strong>Crossing play at 9m</strong> — centre-back and a back player cross paths to force a defensive switch; the player receiving after the cross has a half-second of separation to shoot. Diagram the cross paths and the pivot's screen to delay pursuit.</li>
<li><strong>Pivot screen</strong> — the pivot (PV) sets a body screen on the defender covering the shooting back player; show the timing relative to the ball position.</li>
...
<p>The fast break is handball's highest-percentage scoring opportunity — a transition directly from defense to a numerical advantage before the opposition defense can reset:</p>
<ul>
...
<p>The beach handball court (27×12m) is roughly half the size of an indoor court, giving defenders much less time to react. The standard defensive alignment is a <strong>2-1 zone</strong> — two defenders flanking the goalkeeper area, one defender covering the front zone. Diagram the spacing responsibilities, the lateral slide triggers, and how the zone adjusts when the ball goes wide to the corner.</p>
<p>Against teams that favour spin shots, the defending player must stay tight without fouling — a balance that requires specific positioning: arms up, body balanced, and one step closer to the thrower than the standard gap.</p>
<h2>Fast Break and Counter-Attack</h2>
<p>Given the court's small size, fast breaks in beach handball are even more explosive than in the indoor game. Diagram:</p>
<ul>
...
<p>Set plays from free throws are critical in beach handball because the compact court means even a marginal advantage in positioning creates a quality shot. Design a three-player combination: one player sets a screen for the thrower, one cuts behind the screen to receive the pass, and the third occupies the goalkeeper's angle. Draw all three players' movements simultaneously on CourtDraw so the coordination becomes clear before the players walk through it on court.</p>''',
  },
  {
    ...
    'plays': [
      ...
      {'id':'basketball_inbound','name':'Baseline Inbound','desc':'BLOB set creating a catch-and-shoot or drive option from baseline out-of-bounds.'},
    ],
...
<h2>Zone Defense</h2>
<p>Zone defense confuses attackers who've only been taught to beat man-to-man. The <strong>2-3 zone</strong> is the most common: two guards on the perimeter, three players across the paint. Teams attack the 2-3 by placing a shooter in the high post and hitting the corners rapidly. Diagram the defensive rotations required to close this gap.</p>
<p>The <strong>1-3-1 zone</strong> is more aggressive — one player pressures the ball at the top, three across the middle level, one at the basket. It creates turnovers but is vulnerable in the corners. Draw the attack pattern (swing to corner, then swing back to opposite corner for a shot) alongside the defensive recovery rotation.</p>
...
<p><strong>Transition defense</strong> — getting back in position before the opponent can score — requires diagrammed responsibilities: who sprints back first, who pressures the outlet pass, and who protects the paint.</p>
...
<li><strong>Double lead</strong> — both GA and WA make simultaneous drives in different directions, forcing the opponent's defensive pair to choose one.</li>
...
<p>Draw each pattern showing the starting positions, the movement arrows, the pass timing, and the court zone boundaries that constrain each movement.</p>
<h2>Circle Edge Attack</h2>
<p>The shooting circle edge is where most attacking play is created. Key circle-edge principles to diagram:</p>
<ul>
...
<p>Netball defense is primarily man-to-man, but the three-foot rule (defenders must stand at least three feet from the ball-holder) creates specific coaching challenges. Diagram the correct defensive position and show how it shifts when the ball is passed — the defender must rotate immediately to achieve correct positioning on the new ball-holder within three seconds.</p>
<p>The <strong>defensive press</strong> — applying pressure from the centre circle outward to disrupt the centre pass — involves all players in their zones attempting to intercept or disrupt the first two passes. Draw the press triggers, the recovery positions if the press fails, and the off-ball interception positions for WD, GD, and GK.</p>''',
  },
  {
    'slug': 'korfball',
    'name': 'Korfball',
    ...
    'family': 'hand',
    'plays': [],
    'intro': "<p>Korfball's mixed-gender 8v8 format and fixed zone system create unique tactical challenges rarely found in other sports. The court splits into two identical zones — each with four attackers and four defenders who swap roles after two goals — and coaching the timing of that zone transition alongside attacking combination plays demands clear visual diagrams. CourtDraw's korfball board covers both zones with accurate post positions.</p>",
    'body': '''<h2>The Korfball System</h2>
<p>The court is divided into two equal zones, one at each end, each containing a post. Four players from each team are assigned to each zone. After every two goals scored (by either team), all players switch zones, meaning attackers become defenders and vice versa. This zone-switching requirement makes tactical flexibility essential.</p>
<h2>Attacking Patterns</h2>
<p>In the attacking zone, your four players must create shooting opportunities against four defenders:</p>
<ul>
<li><strong>Defender-dodging</strong> — the attacker must get the defender's back to the post to create a legal shooting position. Show the approach angle, the step-across body movement, and the resulting shooting position.</li>
<li><strong>Penalty shot</strong> — awarded when a defender illegally obstructs a clear shot; diagram the positioning requirements (2.5m in front of the post, one attempt).</li>
...
<p>The two-goal zone switch is a distinctive tactical moment. Diagram the communication and positioning protocol: which players cross first, how to communicate the switch mid-play, and the initial positioning each player should take in the new zone. Teams that transition quickly and decisively often score in the first thirty seconds after a switch because the newly attacking players are more aggressive while the newly defensive players are still orienting.</p>''',
  },
  {
    ...
    'metaDesc': 'Free online water polo tactics board for coaches. Draw 6v5 power play sets, man-up offenses, 2-meter player positioning, and goalkeeper angles. Works on iPad.',
...
<p>The exclusion foul and resulting man-up is the most tactically structured situation in water polo. Elite teams run rehearsed power-play sets that create high-percentage shooting positions before the excluded defender returns. The standard <strong>arc formation</strong>:</p>
<ul>
...
<p>The power play lasts 20 seconds — every player must know their role immediately.</p>
<h2>6v6 Half-Court Offense</h2>
<p>In even-strength situations, water polo offense is built around the 2-metre player. Diagram:</p>
<ul>
...
<p>The two most common serving/setting systems — the <strong>5-1</strong> (one setter, five hitters) and the <strong>6-2</strong> (two setters, three active hitters) — have entirely different rotation implications. In a 5-1 system, the setter is in the right-back position when serving, which creates a three-passer serve receive in the front court. In a 6-2, the setter coming from the back row must navigate around the libero. Diagram both systems showing which players are where in each of the six rotations.</p>
<h2>Serve Receive Formations</h2>
<p>The serve receive formation determines how much court each passer covers. Three common formations to diagram:</p>
<ul>
<li><strong>W-formation (five passers)</strong> — the traditional serve receive with three passers in a diagonal row and two players near the net free to attack immediately.</li>
<li><strong>Three-passer system</strong> — in a 5-1 with a strong libero, only three players pass and three are free at the net. Diagram the specific coverage zones.</li>
<li><strong>Two-passer system</strong> — limiting passing to two specialised players; show their movement patterns to cover the entire court from starting positions.</li>
</ul>
<h2>Attack Patterns by Rotation</h2>
<p>Which attack options are available depends on the rotation. In rotation 1 (setter in right back), the outside hitter is in the front left, the middle is front centre, and the opposite is in the right front — three front-row attackers are available. Diagram the primary attack call and the back-row attack options for each rotation. Teams that understand their attack options in every rotation make better play-call decisions under pressure.</p>
<h2>Blocking Systems</h2>
<p>Two blocking systems are used in elite volleyball:</p>
<ul>
<li><strong>Reading blocking</strong> — blockers read the setter's hands and the hitter's approach angle before committing to a block position.</li>
...
<p>Diagram the blocker's angle relative to the antenna and the coverage zone each defender covers behind the block. The gap between the block and the sideline is the most common undefended area — show which player is responsible.</p>
...
<p>Show the attacker's view: the blocker's hand position tells the attacker which option is open. Draw the blocker's hands blocking line, and the resulting open angle. This visual decision tree takes seconds to create and permanently improves attack selection.</p>
<h2>Blocking and Defensive Partnership</h2>
<p>Because there are only two players, blocking requires a precise agreement between partners:</p>
<ul>
...
<li><strong>No-block (peel defense)</strong> — the blocker drops back to defend; both players cover the court without a block. Effective against players with excellent off-speed shots.</li>
<li><strong>Pointer system</strong> — the blocker signals to the setter before the serve which direction they'll block, allowing the defender to pre-position. Draw the hand signal and the resulting defensive coverage as two separate diagrams.</li>
</ul>
...
<p>The serve in fistball is a jump or standing punch aimed to make accurate receive difficult. Show target zones:</p>
<ul>
...
<h2>Build-Up Play</h2>
<p>Coaching build-up from the back requires players to understand the spacing principles that create passing lanes. Diagram:</p>
<ul>
...
<p>Futsal's attacking structure is built around continuous rotation. The standard 1-2-1 formation places one fixo (defender), two alas (wingers), and one pivô (centre forward). But the tactical power is in the rotation: as the ball moves, each player shifts into a new role fluidly. Draw the 1-2-1 rotation as a sequential animation: ball to the right ala → ala passes to the pivô → pivô lays off to the arriving left ala → left ala shoots or returns to fixo. Show each player's movement path as the ball progresses.</p>
<h2>Pivot Play</h2>
<p>The <strong>pivô</strong> is futsal's most influential position — a physically strong, technically precise player who operates with their back to goal. Key pivô plays to diagram:</p>
<ul>
...
<p>Because the court is small and the ball stays live off the boards, pressing in futsal is extremely effective. Three main pressing shapes:</p>
<ul>
...
<p>With five players and no offside rule, beach soccer attack is fluid and vertical:</p>
<ul>
...
<li><strong>Goalkeeper counter-throw</strong> — the goalkeeper's long throw directly to a striker running in behind the defense; diagram the striker's run angle and the timing of the release.</li>
</ul>
<h2>Free Kicks and Set Pieces</h2>
<p>Free kicks in beach soccer are among the most entertaining set pieces in sport — specialists can curl, dip, and rainbow the ball over walls. Diagram the wall position, the fake run to distract the wall, and the intended ball flight path. Also show the secondary option: a pass around the wall to a player who volleys first-time while the goalkeeper is committed to the direct shot.</p>
...
<li><strong>Number 8 pick-and-go</strong> — number 8 picks the ball from the base of the scrum and drives directly into the defensive line; diagram the forwards' pod formation ahead of the pick.</li>
...
<p>Modern rugby union attack is built around creating width and then exploiting the overlaps that the defense's rush creates:</p>
<ul>
<li><strong>Pod attack</strong> — forward pods of three hit the defense in succession, each taking the ball to contact and recycling; show the pods' positions relative to the previous ruck.</li>
...
<li><strong>Drift defense</strong> — the line holds and forces the ball wide toward the touchline rather than advancing; diagram the drift angle for each defender relative to their opposite number's inside shoulder.</li>
  ...
  },
  {
    ...
    'family': 'football',
    'plays': [],
    'intro': "<p>Rugby league's six-tackle possession structure creates a very different tactical environment from union — every set of six is a managed sequence of play, a kick return, and a defensive reset. CourtDraw's rugby league board helps coaches design dummy-half plays, diagram the kick-on-last game plan, and draw defensive line spacing before the session, so players arrive understanding the week's tactical focus.</p>",
    'body': '''<h2>Six-Tackle Structure</h2>
<p>Every set of six tackles in rugby league should be a planned sequence. Coaches diagram the six tackles as a progression:</p>
<ul>
...
<p>The <strong>dummy-half play</strong> on tackle 3 or 4 is especially effective when the defense has settled into a flat line and the gap is between the marker and the first defender. Diagram this as a two-phase play: the carry into contact on tackle 3, the dummy-half spotting the gap, and the run on the resulting play-the-ball.</p>
<h2>Kick Game</h2>
...
<p>Rugby league defense is a line — 13 players closing the width of the field in coordination. Show the standard defensive alignment (one marker, two flanking the ruck, line defense across the width) and the communication protocols when the line must shift for a wide attack. The most common defensive error — gaps forming between adjacent defenders — is corrected more effectively with a visual diagram than with verbal instruction.</p>''',
  },
  {
    ...
    'metaDesc': 'Free online gaelic football tactics board for coaches. Draw kickout strategies, sweeper systems, half-forward play, and set piece routines. Works on iPad.',
...
<p>The sweeper system — placing one player as a deep defender covering the space behind the main defensive line — has been a defining tactical debate in Gaelic football. Diagram it from both perspectives:</p>
<ul>
...
<p>A passing play is more than one route — it's a concept where multiple routes combine to attack the same defensive zone from different angles:</p>
<ul>
<li><strong>Mesh concept</strong> — two receivers cross paths in the middle of the field; the QB reads which receiver comes open first based on the linebacker's decision to follow which crosser.</li>
...
<p>Defensive coverage is the chess game within the game:</p>
<ul>
...
<li><strong>Cover 3</strong> — three defenders cover deep thirds; four defenders cover short zones. Vulnerable to the 4-verticals concept when four receivers flood the three deep zones.</li>
<li><strong>Man coverage</strong> — each defender covers a specific receiver regardless of their route; show the leverage position for each defensive assignment.</li>
</ul>
...
<p>The centre bounce is AFL's equivalent of a set piece — 36 players position themselves across the oval before the umpire bounces the ball at centre. The four players in the centre square contest the bounce while the remaining 28 position to best receive the cleared ball. Diagram your setup:</p>
<ul>
...
<h2>Zone Defense</h2>
<p>Zone defense in AFL — players covering areas of the ground rather than man-marking opponents — has become increasingly common:</p>
<ul>
...
<li><strong>Short corner</strong> — a pass variation; instead of a direct shot, the stopper passes sideways to a second player who shoots first-time from a different angle. Show the second shooter's starting position and run path.</li>
</ul>
...
<p>The standard 3-5-2 (three defenders, five midfielders, two forwards) creates defensive compactness while giving the midfield freedom to both support attack and press defensively. Diagram the 3-5-2's key spatial relationships:</p>
<ul>
...
<p>The high press in field hockey is most effective from corner presses after a sideline deflection or when the ball is played back to the goalkeeper. The key principles to diagram:</p>
<ul>
...
<h2>Circle Attack</h2>
<p>Creating circle entries is the central challenge of field hockey attacking play. Three common entry patterns:</p>
<ul>
...
<li><strong>Direct breakout</strong> — defenseman retrieves and passes directly to the far-side winger or to the centre who drops into support. Diagram all three outlet pass options and the conditions under which each is preferred.</li>
</ul>
...
<li><strong>2-3 trap</strong> — two forwards forecheck lightly, three players hold the neutral zone. Most effective when protecting a lead late in a game.</li>
</ul>
...
<li><strong>Man-to-man</strong> — each defender follows their assigned player; show the assignments by number and how these switch after a line change.</li>
</ul>''',
  },
  {
    'slug': 'roller-hockey',
...
<p>A 4v3 power play in roller hockey creates significant scoring pressure. The most effective formation places one player at the top of the zone (point) and two players in the half-space flanking the goal, with the fourth player near the net:</p>
<ul>
...
<p>The standard indoor hockey formation is 1-2-2 (goalkeeper, two defenders, two midfielders, two forwards). Draw the formation and show the specific coverage responsibilities for each position:</p>
<ul>
...
<p>The standard floorball formation is 2-2-1: two defenders, two midfielders, one forward. Diagram the formation not as fixed positions but as a dynamic shape:</p>
<ul>
...
<ul>
<li><strong>1-3-1 formation</strong> — one player at the top, three across the middle (two half-wall, one in the hole), one in front of the goal. Diagram the rotation when the top player moves to shoot.</li>
...
<h2>Fast Break</h2>
<p>Floorball's most productive transitions come from fast breaks — quick counter-attacks before the opponent's defense resets. Show the two most common scenarios:</p>
<ul>
...
<p>Also diagram the penalty shot rules — awarded after a clear scoring opportunity is fouled — and the goalkeeper's positioning and technique for facing a penalty shot.</p>''',
  },
  {
    ...
    'metaDesc': 'Free online hurling and camogie tactics board for coaches. Draw puckout strategies, sweeper systems, forward line positioning, and sideline plays. Works on iPad.',
...
<p>The sweeper system — a half-back who drops behind the midfield line to clog the space between the half-forward and full-back lines — has transformed tactical hurling. Diagram the sweeper's positioning responsibilities:</p>
<ul>
...
<h2>Fast Break</h2>
<p>Lacrosse fast breaks are among the most dynamic and highest-percentage scoring opportunities in the sport. A 3v2 fast break should be taught as a structured play:</p>
<ul>
...
def get_sport_name(slug):
  s = next((x for x in SPORTS if x['slug'] == slug), None)
  return s['name'] if s else slug

def get_related_links(sport, count=5):
  family = FAMILIES[sport['family']]
  same = [s for s in family['slugs'] if s != sport['slug']][:count-1]
  other = [s for fk, fv in FAMILIES.items() if fk != sport['family']
             for s in fv['slugs'] if s not in same and s != sport['slug']]
...
NAV_HTML = '''<nav>
  <div class="container nav-inner">
    ...
    <div class="nav-cta">
      ...
      <a href="/courtdraw-app.html?court={court}" class="btn btn-primary" style="padding:9px 18px;font-size:.875rem;">Try Free</a>
    </div>
  </div>
</nav>'''.replace('{logo}', LOGO_SVG)
  ...
  <div class="container">
    <div class="footer-grid">
      ...
      <div class="footer-col">
        ...
        <ul>
          ...
          <li><a href="mailto:courtdraw.info@gmail.com">Contact</a></li>
        </ul>
      </div>
    </div>
    <div class="footer-bottom">
      ...
      <div class="footer-legal">
        ...
        <a href="mailto:courtdraw.info@gmail.com">Contact</a>
      </div>
    </div>
  </div>
</footer>'''.replace('{logo}', LOGO_SVG)

def build_json_ld(sport, url):
  data = {
    "@context": "https://schema.org",
    "@graph": [
      {
        "@type": "WebApplication",
        "name": f"CourtDraw — {sport['name']} Tactics Board",
      ...
      },
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": f"Is there a free {sport['faqSport']} tactics board?",
            "acceptedAnswer": {"@type": "Answer", "text": f"Yes — CourtDraw is completely free to start. Open the {sport['name']} tactics board in your browser at courtdraw.app, no account or install required. The free plan gives you access to one court and three saved tactics. Pro unlocks all 38+ sports, unlimited saves, and shareable links."},
          },
          {
            "@type": "Question",
            "name": f"How do I draw {sport['faqSport']} plays online?",
            "acceptedAnswer": {"@type": "Answer", "text": f"Open CourtDraw, select the {sport['name']} court, and use the drawing tools to drag player tokens, draw arrows for passes and runs, and add circles and zones. Save your tactic, then share it via a link or export as PNG or PDF. No drawing experience required."},
          },
          {
            "@type": "Question",
            "name": f"Does the {sport['faqSport']} tactics board work on iPad and offline?",
            "acceptedAnswer": {"@type": "Answer", "text": f"Yes. CourtDraw is a Progressive Web App (PWA) that works on any browser including iPad Safari and Chrome. Once loaded, it works fully offline — diagrams and saved tactics are stored on the device. Add it to your home screen for instant access on the touchline."},
          },
        ],
      },
    ],
...
def build_page(sport):
  url = f"https://courtdraw.app/{sport['slug']}-tactics-board/"
  primary_court = sport['primaryCourtId']
  related = get_related_links(sport, 5)

  court_list = ''
  consolidation_note = ''
  if len(sport['courtIds']) > 1:
    names = ' and the '.join(n for _, n in sport['courtIds'])
    court_list = f'<p>This page covers the <strong>{names}</strong>. All court variants are available in the app with a single tap.</p>'
...
<section class="sp-section" style="background:linear-gradient(135deg,rgba(29,78,216,0.08),rgba(59,130,246,0.04));border-top:1px solid rgba(59,130,246,0.2);border-bottom:1px solid rgba(59,130,246,0.2);padding:56px 0;">
  ...
  if sport['plays']:
    cards = '\n'.join(f'''        <div class="play-card">
          ...
          <a href="/courtdraw-app.html?court={primary_court}" class="play-load-btn">Load in Board →</a>
        </div>''' for p in sport['plays'])
    plays_html = f'''
  ...
  related_links = '\n      '.join(
    f'<a href="/{slug}-tactics-board/" class="related-link">{get_sport_name(slug)} Tactics Board</a>'
    for slug in related
  )

  nav = NAV_HTML.replace('{court}', primary_court)
...
<head>
  ...
  <meta name="description" content="{escape_html(sport['metaDesc'])}" />
  <link rel="canonical" href="{url}" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="{url}" />
  <meta property="og:title" content="{escape_html(sport['title'])}" />
  <meta property="og:description" content="{escape_html(sport['metaDesc'])}" />
  ...
  <meta name="twitter:title" content="{escape_html(sport['title'])}" />
  <meta name="twitter:description" content="{escape_html(sport['metaDesc'])}" />
  ...
  <script type="application/ld+json">{build_json_ld(sport, url)}</script>
  <style>
{SHARED_CSS}
    ...
    .consolidation-note {{ display: inline-flex; gap: 8px; align-items: center; background: var(--bg2); border: 1px solid var(--border2); border-radius: var(--radius); padding: 8px 16px; font-size: 0.85rem; color: var(--muted); margin-top: 16px; flex-wrap: wrap; justify-content: center; }}
    ...
    .plays-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; }}
...
<section class="sp-hero">
  <div class="container">
    ...
    <div class="sp-hero-actions" style="margin-top:28px;">
      <a href="/courtdraw-app.html?court={primary_court}" class="btn btn-primary btn-lg">Start Drawing {sport['name']} Free →</a>
  ...
  <div class="container">
    ...
    <div class="faq-list">
      ...
      <div class="faq-item">
        ...
        <p>Open CourtDraw, select the {sport['name']} court, and use the drawing tools: drag player tokens, draw solid arrows for passes and runs, dashed arrows for off-ball movement, and add circles and zones. Save your tactic, then share it via a link or export as PNG or PDF. No drawing experience needed.</p>
      </div>
      <div class="faq-item">
...
<section class="sp-cta" style="background:var(--bg2);border-top:1px solid var(--border);border-bottom:1px solid var(--border);">
  <div class="container">
    ...
    <a href="/courtdraw-app.html?court={primary_court}" class="btn btn-primary btn-lg">Open {sport['name']} Tactics Board →</a>
...
def build_hub_page():
  ...
  for fkey, fam in FAMILIES.items():
    sports = [s for slug in fam['slugs'] for s in SPORTS if s['slug'] == slug]
    cards = '\n'.join(
      f'      <a href="/{s["slug"]}-tactics-board/" class="hub-card"><span class="hub-card-icon">{s["emoji"]}</span><span class="hub-card-name">{s["name"]}</span></a>'
      for s in sports
    )
    groups += f'\n    <div class="hub-family">\n      <h2 class="hub-family-title">{fam["label"]}</h2>\n      <div class="hub-family-grid">{cards}\n      </div>\n    </div>'

  json_ld = json.dumps({
    "@context": "https://schema.org",
    "@graph": [
      {"@type": "CollectionPage", "name": "Sports Tactics Boards — CourtDraw",
...
<head>
  ...
  <style>
{SHARED_CSS}
    .hub-hero {{ padding: 72px 0 56px; text-align: center; position: relative; }}
    .hub-hero::before {{ content:''; position:absolute; inset:0; background: radial-gradient(ellipse 80% 60% at 50% -10%, rgba(59,130,246,0.15) 0%, transparent 70%); pointer-events:none; }}
    ...
    .hub-family-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 12px; }}
...
for sport in SPORTS:
  d = os.path.join(BASE, f"{sport['slug']}-tactics-board")
  os.makedirs(d, exist_ok=True)
  with open(os.path.join(d, 'index.html'), 'w', encoding='utf-8') as f:
    f.write(build_page(sport))
  print(f"✓ {sport['slug']}-tactics-board/index.html")

hub_dir = os.path.join(BASE, 'sports')
os.makedirs(hub_dir, exist_ok=True)
with open(os.path.join(hub_dir, 'index.html'), 'w', encoding='utf-8') as f:
  f.write(build_hub_page())
print('✓ sports/index.html')

with open(os.path.join(BASE, 'sitemap.xml'), 'w', encoding='utf-8') as f:
  f.write(build_sitemap())
print('✓ sitemap.xml')

print(f'\n✅ Done — {len(SPORTS)} sport pages + hub page + sitemap.')
no_plays = [s['name'] for s in SPORTS if not s.get('plays')]
print(f'Sports without sample plays ({len(no_plays)}):')
for n in no_plays:
  print(f'  - {n}')
